...
testPredictionDir = "./mammal45/test"
trainPredictionDir = "./mammal45/train"
models = ["alexNet", "Densenet", "Resnet_20", "Resnet_32", "Resnet_44", "Resnet_56", "Resnet_110", "Vgg16_bn", "Vgg19_bn", "Wrn_28"]
testSuffix = "_test_prediction.pt"
trainSuffix = "_train_prediction.pt"
...
tmpAccList = list()
for m in models:
    predictionPath = os.path.join(testPredictionDir, m+testSuffix)
    prediction = torch.load(predictionPath, map_location=torch.device("cpu"))
    ...
    print(f"{m:12s}{tmpAccList[-1][0].item():.4f}%")
...
for m in models:
    trainPredictionPath = os.path.join(trainPredictionDir, m+trainSuffix)
    trainPrediction = torch.load(trainPredictionPath, map_location=torch.device("cpu"))
...
for j, dm in enumerate(diversityMetricsList):
    FQMetricsThreshold[dm] = np.mean(list(FQMetrics[dm].values()))
...
for dm in diversityMetricsList:
    print(dm, list(teamSelectedFQMeanDict[dm]))
print()

# Hierarchical Pruning
targetTeamSize = 5

toPruneUpperPercentage = 0.27
toPruneList = []
FQAccTeamNamesDict = {}
OutFQAccTeamNames = {}
teamSelectedPrunedFQSet = set()
for teamSize in range(2, targetTeamSize+1):
    print("Team Size", teamSize, " Report")
    FQAccDMDict = {}
    for dm in diversityMetricsList:
        tmpFQAccTeamNames = []
        tmpOutFQAccTeamNames = []
        for teamName in teamSizeDict[teamSize]:
            if isTeamContainsAny(teamName, toPruneList):
        ...
        FQAccDMDict[dm] = sorted(tmpFQAccTeamNames, key=itemgetter(0), reverse=True)
        
    toPruneUpperNum = int(len(tmpFQAccTeamNames) * toPruneUpperPercentage)

    tPSize = min(toPruneUpperNum, len(tmpFQAccTeamNames))
    for tPIdx in range(tPSize):
        count = 0
        for idx in range(len(diversityMetricsList)-1):
            if FQAccDMDict[diversityMetricsList[-1]][tPIdx][2] in [x[2] for x in FQAccDMDict[diversityMetricsList[idx]][:tPSize]]:
                count += 1
        if count == 3:
            toPruneList.append(FQAccDMDict[diversityMetricsList[-1]][tPIdx][2])
            tmpOutFQAccTeamNames.append(FQAccDMDict[diversityMetricsList[-1]][tPIdx])
            tmpFQAccTeamNames.remove(FQAccDMDict[diversityMetricsList[-1]][tPIdx])
    if len(tmpFQAccTeamNames) == 0:
        print("No in")
    else:
        tmpAcc = [fqATN[1] for fqATN in tmpFQAccTeamNames]
        selectedTeamNameList = [fqATN[2] for fqATN in tmpFQAccTeamNames]
        teamSelectedPrunedFQSet.update(selectedTeamNameList)
        print("In Accuracy Range: ", min(tmpAcc), max(tmpAcc), getNTeamStatistics(selectedTeamNameList, teamAccuracyDict, minAcc, avgAcc, maxAcc, tmpAccList))
    if len(tmpOutFQAccTeamNames) == 0:
        print("No out")
    else:
        tmpOutAcc = [fqOATN[1] for fqOATN in tmpOutFQAccTeamNames]
        outTeamNameList = [fqOATN[2] for fqOATN in tmpOutFQAccTeamNames]
        print("Out Accuracy Range: ", min(tmpOutAcc), max(tmpOutAcc), getNTeamStatistics(outTeamNameList, teamAccuracyDict, minAcc, avgAcc, maxAcc, tmpAccList))
...
TeamCountDict = dict()
for dm in diversityMetricsList:
    for selected in teamSelectedPrunedFQSet:
        if selected in TeamCountDict:
            TeamCountDict[selected] += 1
        else:
            TeamCountDict[selected] = 1
for teamName, count in TeamCountDict.items():
    if count >= 3 and len(teamName) == targetTeamSize:
        SelectedTeamList.append((teamName, teamAccuracyDict[teamName]))
print(SelectedTeamList)